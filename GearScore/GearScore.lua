...
function GearScore_OnEvent(GS_Nil, GS_EventName, GS_Prefix, GS_AddonMessage, GS_Whisper, GS_Sender)
    ...
    if (GS_EventName == "EQUIPMENT_SWAP_PENDING") then
        GS_PlayerIsSwitchingGear = true;
        GS_PlayerSwappedGear = 0
        return;
    end
    if (GS_EventName == "EQUIPMENT_SWAP_FINISHED") then
        ...
        PersonalGearScore:SetText(GS_Data[GetRealmName()].Players[UnitName("player")].GearScore);
    ...
    if (GS_EventName == "CHAT_MSG_CHANNEL") then
        local Who = GS_AddonMessage;
        local Message = GS_Prefix;
        local ExtraMessage = "";
        local ColorClass = "";
        local Channel = GS_Sender
        if GS_Data[GetRealmName()].Players[Who] then
            ColorClass = "|cff" ..
                             string.format("%02x%02x%02x", GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Who]
                    .Class]].Red * 255,
                    GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Who].Class]].Green * 255,
                    GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Who].Class]].Blue * 255)
        ...
        if string.find(Channel, "Trade") then
            -- print("StringFound!")
            local A, B = string.find(Channel, "Trade");
            Channel = string.sub(Channel, 1, B)
        end
        Channel = "[" .. Channel .. "] "

        local NewMessage = Channel .. ExtraMessage .. "|Hplayer:" .. Who .. "|h[" .. ColorClass .. Who .. "|r]|h: " ..
                               Message
        -- print(NewMessage)
    ...
    if (GS_EventName == "INSPECT_ACHIEVEMENT_READY") then
        ...
        if (GS_DisplayFrame:IsVisible()) then
            GS_DisplayXP(UnitName("target")); -- GearScoreClassScan(UnitName("target"));
        end
    end

    if (GS_EventName == "PLAYER_EQUIPMENT_CHANGED") then

        if (GS_PlayerIsSwitchingGear == true) then
            GS_PlayerSwappedGear = GS_PlayerSwappedGear + 1
            return;
        end
        if (GS_PlayerSwappedGear) then
            GS_PlayerSwappedGear = GS_PlayerSwappedGear - 1;
            if (GS_PlayerSwappedGear == 0) then
                GS_PlayerSwappedGear = nil;
            end
            return;
        end
        GearScore_GetScore(UnitName("player"), "player");
    ...
    if (GS_EventName == "CHAT_MSG_ADDON") then
        if not (GS_Whisper == "GUILD") then
            return;
        end
        if GS_Settings["BlackList"] then
            if GS_Settings["BlackList"][GS_Sender] then
                return;
            end
        end
        if not (GearScoreChatMessageThrottle) then
            GearScoreChatMessageThrottle = 0;
        end
        ...
        if (GS_Prefix == "GSY_Version") and (tonumber(GS_AddonMessage)) and (GS_Settings["OldVer"]) then
            if (tonumber(GS_AddonMessage) > GS_Settings["OldVer"]) then
                print("There is a newer version of GearScore. Go to www.GearScoreAddon.com to update.");
        ...
        if (GS_Prefix == "GSY_Request") and (GS_Settings["Communication"] == 1) and (GS_Sender ~= UnitName("player")) then
            ...
            if (GearScoreChatMessageThrottle >= 1) then
                return;
            end
            if (GS_Data[GetRealmName()].Players[GS_AddonMessage]) then
                GearScore_Send(GS_AddonMessage, "GUILD", GS_Sender);
            end
        end
        if (GS_Prefix == "GSY") and (GS_Settings["Communication"] == 1) and (GS_Sender ~= UnitName("player")) then
            if (GS_Whisper == "RAID") then
                GS_Whisper = "PARTY";
            end
            local tbl = {}
            ...
            if (tbl[1] == UnitName("player")) or ((tbl[11] ~= GS_Sender) and (tbl[11] ~= " ")) then
                return;
            end
            if (tbl[1]) and (tbl[2]) and (tbl[3]) then
                ...
                if not (GS_Data[GetRealmName()].Players[tbl[1]]) then
                    ...
                    if TestAuthenticity then
                        return
                    end
                    if (UnitName("mouseover") == tbl[1]) then
                        GameTooltip:SetUnit(tbl[1]);
                    end
                    if (GS_DisplayPlayer == tbl[1]) and (GS_DisplayFrame:IsVisible()) then
                        GearScore_DisplayUnit(tbl[1], 1);
                    end
                    if ((GS_Factions[GS_Data[GetRealmName()].Players[tbl[1]].Faction] ~= UnitFactionGroup("player")) and
                        (GS_Settings["KeepFaction"] == -1)) or
                        ((GS_Data[GetRealmName()].Players[tbl[1]].Level < GS_Settings["MinLevel"]) and
                            (tbl[1] ~= UnitName("player"))) then
                ...
                if not (tonumber(GS_Data[GetRealmName()].Players[tbl[1]].Date) >= tonumber(tbl[3])) then
                    if (tonumber(tbl[3]) > GearScore_GetTimeStamp()) then
                        return;
                    end
    ...
    if (GS_EventName == "ADDON_LOADED") then
        if (GS_Prefix == "GearScore") then
            ...
            GS_Settings["Developer"] = 0;
            GS_VersionNum = 30115;
            GS_Settings["OldVer"] = GS_VersionNum
            ...
            if (GS_Settings["Developer"] == 0) then
                print("Welcome to GearScore 3.1.15. Target a player and type /gs for new advanced features.");
            end
            if (GS_Settings["Developer"] == 1) then
                print(
                    "Welcome to GearScore 3.1.15. This is a test version. Please provide feedback at www.GearScoreAddon.com");
            end
            if (GS_Settings["Restrict"] == 1) then
                GearScore_SetNone();
            end
            if (GS_Settings["Restrict"] == 2) then
                GearScore_SetLight();
            end
            if (GS_Settings["Restrict"] == 3) then
                GearScore_SetHeavy();
            end
            ...
            GearScore_GetScore(UnitName("player"), "player");
            GearScore_Send(UnitName("player"), "ALL")
            if (GetGuildInfo("player")) and (GS_Settings["Developer"] ~= 1) then
        ...
        if (GS_Prefix == "GearScoreRecount") then
            ...
            local s = GearScoreRecountWarning;
            s:SetFont("Fonts\\FRIZQT__.TTF", 30);
            s:SetText("WARNING! GearScoreRecount MUST be disabled to use GearScore. 3.1.10")
            s:SetPoint("BOTTOMLEFT", UIParent, "CENTER", -600, 200)
            s:Show();
            f:Show()
            print(
                "WARNING! GearScoreRecount MUST be disabled to use GearScore. Please turn it off or remove it from your addons folder, Sorry for the inconvience")
...
function GearScore_ComposeRecord(tbl, GS_Sender)
    local Name, GearScore, Date, Class, Average, Race, Faction, Location, Level, Sex, Guild, Scanned, Equip = tbl[1],
        tonumber(tbl[2]), tonumber(tbl[3]), tbl[4], tonumber(tbl[5]), tbl[6], tbl[7], tbl[8], tostring(tbl[9]), 1,
        tbl[10], GS_Sender, {}
    --	print(Name, GearScore, Date, Class, Average, Race, Faction, Location, Level, Sex, Guild, Scanned)
    if Scanned == "LOLGearScore" or Scanned == "GearScoreBreaker" then
        return "InValid";
    end
    if (Scanned == " ") then
        Scanned = "Unknown";
...
function GearScore_Prune()
    ...
    for i, v in pairs(GS_Data[GetRealmName()].Players) do
        ...
        if ((GS_Factions[v.Faction] ~= UnitFactionGroup("player")) and (GS_Settings["KeepFaction"] == -1)) or
            ((tonumber(v.Level) < GS_Settings["MinLevel"]) and (v.Name ~= UnitName("player"))) then
            GS_Data[GetRealmName()].Players[v.Name] = nil;
        end
        if not v.GearScore or not v.Name or not v.Sex or not v.Equip or not v.Location or not v.Level or not v.Faction or
            not v.Guild or not v.Race or not v.Class or not v.Date or not v.Average or not v.Scanned then
            GS_Data[GetRealmName()].Players[i] = nil;
        end
        if (string.find(v.Scanned, "<")) then
            GS_Data[GetRealmName()].Players[v.Name] = nil;
        end
        if v.Guild == "<>" or v.Guild == "" then
            GS_Data[GetRealmName()].Players[v.Name].Guild = "*";
        end
        if (string.find(v.Guild, "<")) then
            GS_Data[GetRealmName()].Players[v.Name].Guild = string.sub(v.Guild, 2, strlen(v.Guild) - 1);
        end
        if ((type(tonumber(v.Guild))) == "number") then
            GS_Data[GetRealmName()].Players[v.Name] = nil;
        end
        if v.Scanned == "LOLGearScore" then
            GS_Data[GetRealmName()].Players[v.Name] = nil;
        end
...
function GearScore_EquipCompare(Tooltip, ItemScore, ItemSlot, GS_ItemLink)
    ...
    local ItemName, ItemLink, ItemRarity, ItemLevel, ItemMinLevel, ItemType, ItemSubType, ItemStackCount, ItemEquipLoc,
        ItemTexture = GetItemInfo(GS_ItemLink)
    ...
    for i, v in pairs(GSL_DataBase) do
        ...
        if (GS_ClassInfo[GS_Classes[v.Class]].Equip[ItemSubType]) or (ItemEquipLoc == "INVTYPE_CLOAK") then
            if (ItemSlot == 18) and v.Class == "HU" then
                HunterMultiplier = 5.3224;
            end
            if ((ItemSlot == 17) or (ItemSlot == 16) or (ItemSlot == 36)) and (v.Class == "HU") then
                HunterMultiplier = 0.3164;
            end
            ...
            if (ItemSlot > 20) or (ItemSlot == 16) then
                ...
                local ItemName2, ItemLink2, ItemRarity2, ItemLevel2, ItemMinLevel2, ItemType2, ItemSubType2,
                    ItemStackCount2, ItemEquipLoc2, ItemTexture2 = GetItemInfo("item:" .. v.Equip[ItemSlot - 20])
                local ItemName3, ItemLink3, ItemRarity3, ItemLevel3, ItemMinLevel3, ItemType3, ItemSubType3,
                    ItemStackCount3, ItemEquipLoc3, ItemTexture3 = GetItemInfo("item:" .. v.Equip[ItemSlot - 19])
                if (ItemLink2) then
                    ItemScore2 = GearScore_GetItemScore(ItemLink2);
                else
                    ItemScore2 = 0;
                end
                if (ItemLink3) then
                    ItemScore3 = GearScore_GetItemScore(ItemLink3);
                else
                    ItemScore3 = 0;
                end
                if (ItemScore2 > ItemScore3) then
                    CompareScore = ItemScore3;
                else
                    CompareScore = ItemScore2;
                end
                -- if ( ItemEquipLoc == "INVTYPE_2HWEAPON" ) then
                ...
                local ItemName2, ItemLink2, ItemRarity2, ItemLevel2, ItemMinLevel2, ItemType2, ItemSubType2,
                    ItemStackCount2, ItemEquipLoc2, ItemTexture2 = GetItemInfo("item:" .. v.Equip[ItemSlot])
                if (ItemLink2) then
                    ItemScore2 = GearScore_GetItemScore(ItemLink2);
                else
                    ItemScore2 = 0;
            ...
            if (Percent > 99.99) or (v.GearScore == (4 / 0)) then
                Percent = 99.99;
            end
            Table[Count] = {
                ["Name"] = v.Name,
    ...
    for i, v in ipairs(Table) do
        ...
        Tooltip:AddDoubleLine(v.Name, X .. v.Percent .. "% (" .. X .. v.Difference .. ")", GS_ClassInfo[v.Class].Red,
            GS_ClassInfo[v.Class].Green, GS_ClassInfo[v.Class].Blue, Red, Green, Blue)
    end
    for i = 1, PartySize do
        if (UnitName(GS_Group .. i)) and not (GS_Data[GetRealmName()].Players[UnitName(Group .. i)]) and
            not (GS_ClassInfo[GS_Data[GetRealmName()].Players[UnitName(Group .. i)].Class].Equip[ItemSubType]) then
            Tooltip:AddDoubleLine(UnitName(GS_Group .. i), "No Data", GS_ClassInfo[UnitClass(GS_Group .. i)].Red,
                GS_ClassInfo[UnitClass(GS_Group .. i)].Green, GS_ClassInfo[UnitClass(GS_Group .. i)].Blue, 1, 1, 1);
        end
    end
end
...
function GearScore_GetItemScore(ItemLink)
    ...
    local TokenLink, TokenNumber = GearScore_GetItemCode(ItemLink)
    if (GS_Tokens[TokenNumber]) then
        return GS_Tokens[TokenNumber].ItemScore, GS_Tokens[TokenNumber].ItemLevel, GS_Tokens[TokenNumber].ItemSlot;
    end
    if (GS_ItemTypes[ItemEquipLoc]) then
        ...
        if (ItemRarity >= 2) and (ItemRarity <= 4) then
            local Red, Green, Blue = GearScore_GetQuality((floor(
                ((ItemLevel - Table[ItemRarity].A) / Table[ItemRarity].B) * 1 * Scale)) * 12.25)
...
function GearScore_Request(GS_Target)
    ...
    if (GearScoreChatMessageThrottle >= 5) then
        return;
    end
    if (GS_Settings["Communication"] == 1) then
        if (GetGuildInfo("player")) then
            SendAddonMessage("GSY_Request", GS_Target, "GUILD"); -- SendAddonMessage( "GSY_Version", GS_Settings["OldVer"], "GUILD"); 
        end
    end
end
-------------------------------------------------------------------------------
...
function GearScore_Send(Name, Group, Target)
    if Group == "RAID" then
        Group = "GUILD";
    end -- Command to convert info to only Guild Channel
    if not (GearScoreChatMessageThrottle) then
        GearScoreChatMessageThrottle = 0;
    end
    if (GearScoreChatMessageThrottle >= 1) then
    ...
    if (GS_Settings["Communication"] == 1) then
        ...
        if (GS_Settings["Developer"]) then
            GS_TempVersion = "";
        end
        if (Name) and (GS_Data[GetRealmName()].Players[Name]) then
            local A = GetRealmName()
            GS_MessageA = GS_Data[A].Players[Name].Name .. "$" .. GS_Data[A].Players[Name].GearScore .. "$" ..
                              GS_Data[A].Players[Name].Date .. "$" .. GS_Data[A].Players[Name].Class .. "$"
            GS_MessageB = GS_Data[A].Players[Name].Average .. "$" .. GS_Data[A].Players[Name].Race .. "$" ..
                              GS_Data[A].Players[Name].Faction .. "$" .. GS_Data[A].Players[Name].Location .. "$"
            ...
            GS_MessageC = GS_Data[A].Players[Name].Level .. "$" .. GS_Data[A].Players[Name].Guild .. "$" ..
                              GS_Data[A].Players[Name].Scanned
            -- print( GS_MessageC )
            GS_MessageD = "$"
            for i = 1, 18 do
                if (i ~= 4) then
        ...
        if not (GS_Length) then
            return;
        end
        if (GS_Length) and (GS_Length < 252) then
            if (Group == "ALL") then
                if (GetGuildInfo("player")) then
                    SendAddonMessage("GSY", GS_MessageA .. GS_MessageB .. GS_MessageC .. GS_MessageD, "Guild");
                end
                SendAddonMessage("GSY", GS_MessageA .. GS_MessageB .. GS_MessageC .. GS_MessageD, "RAID")
            else
                if (Group == "GUILD") and not (GetGuildInfo("player")) then
                    return;
                end
...
function GearScore_GetDate(TimeStamp)
    ...
    local min, hour, day, month, year, GS_Date = 0;
    CopyStamp = TimeStamp;
    meridian = "am"
    local Red, Green, Blue = 0
    year = floor(TimeStamp / 100000000);
    TimeStamp = TimeStamp - (year * 100000000)
    month = floor(TimeStamp / 1000000);
    TimeStamp = TimeStamp - (month * 1000000)
    day = floor(TimeStamp / 10000);
    TimeStamp = TimeStamp - (day * 10000)
    hour = floor(TimeStamp / 100);
    ...
    if (floor(CopyStamp / 10000) == floor(TempDate / 10000)) then
        GS_Date = hour .. ":" .. min .. " " -- ..meridian; 
    end

    -- Add Color!
    local CurrentTime = GearScore_GetTimeStamp()
    local currentyear = floor(CurrentTime / 100000000);
    CurrentTime = CurrentTime - (currentyear * 100000000)
    local currentmonth = floor(CurrentTime / 1000000);
    CurrentTime = CurrentTime - (currentmonth * 1000000)
    local currentday = floor(CurrentTime / 10000);
    CurrentTime = CurrentTime - (currentday * 10000)
    local currenthour = floor(CurrentTime / 100);
    CurrentTime = CurrentTime - (currenthour * 100)
    ...
    if ((currentdays - totaldays) >= 7) then
        Green = 0.9;
        Red = .9;
        Blue = .0;
    end
    if ((currentdays - totaldays) >= 14) then
        Green = .5;
        Red = 1;
        Blue = .25;
    end
    if ((currentdays - totaldays) >= 21) then
        Green = 0;
        Red = 1;
        Blue = 0;
    end
...
function GearScore_GetTimeStamp()
    local GS_Hour, GS_Minute = GetGameTime();
    local monthago = 0
...
function GearScore_HookSetUnit(arg1, arg2)
    ...
    if (CanInspect("mouseover")) and (UnitName("mouseover") == Name) and not (GS_PlayerIsInCombat) then
        ...
        GearScore_GetScore(Name, "mouseover"); -- GS_Data[GetRealmName()]["CurrentPlayer"] = GS_Data[GetRealmName()]["Players"][Name]
        if not (GearScore_IsRecordTheSame(GS_Data[GetRealmName()].Players[Name], PreviousRecord)) then
            GearScore_Send(Name, "ALL");
        end
    end
    if (GS_Data[GetRealmName()].Players[Name]) and (GS_Data[GetRealmName()].Players[Name].GearScore > 0) and
        (GS_Settings["Player"] == 1) then
        local Red, Blue, Green = GearScore_GetQuality(GS_Data[GetRealmName()].Players[Name].GearScore)
        if (GS_Settings["Level"] == 1) then
            GameTooltip:AddDoubleLine("GearScore: " .. GS_Data[GetRealmName()].Players[Name].GearScore,
                "(iLevel: " .. GS_Data[GetRealmName()].Players[Name].Average .. ")", Red, Green, Blue, Red, Green, Blue)
            if (GS_Settings["Date"] == 1) then
                local NoWDate, DateRed, DateGreen, DateBlue = GearScore_GetDate(
                    GS_Data[GetRealmName()].Players[Name].Date);
                GameTooltip:AddLine(NoWDate, DateRed, DateGreen, DateBlue);
            end
        else
            GameTooltip:AddLine("MasterScore: " .. GS_Data[GetRealmName()].Players[Name].GearScore, Red, Green, Blue)
            if (GS_Settings["Date"] == 1) then
                local NoWDate, DateRed, DateGreen, DateBlue = GearScore_GetDate(
                    GS_Data[GetRealmName()].Players[Name].Date);
                GameTooltip:AddLine(NoWDate, DateRed, DateGreen, DateBlue);
        ...
        if ((GS_Factions[GS_Data[GetRealmName()].Players[Name].Faction] ~= UnitFactionGroup("player")) and
            (GS_Settings["KeepFaction"] == -1)) or
            ((GS_Data[GetRealmName()].Players[Name].Level < GS_Settings["MinLevel"]) and (Name ~= UnitName("player"))) then
...
function GearScoreChatAdd(self, event, msg, arg1, ...)
    if (msg == "You aren't in a party.") then
        return true;
    end
    if (GS_ExchangeName) then
        if (msg == "No player named '" .. GS_ExchangeName .. "' is currently playing.") then
    ...
    if (GS_Settings["CHAT"] == 1) then
        ...
        if GS_Data[GetRealmName()].Players[Who] then
            ...
            local ColorGearScore = "|cff" .. string.format("%02x%02x%02x", Red * 255, Blue * 255, Green * 255)
            ExtraMessage = "(" .. ColorGearScore .. tostring(GS_Data[GetRealmName()].Players[Who].GearScore) .. "|r): ";
            ExtraMessage = (ColorGearScore .. "|Hplayer:X33" .. Who .. "|h(" ..
                               tostring(GS_Data[GetRealmName()].Players[Who].GearScore) .. ")|h|r ")
...
function GearScore_OnHookLFG(arg1, arg2, arg3, arg4, arg5)
    ...
    for i = 1, GameTooltip:NumLines() do
        ...
        if (A) and (GS_Data[GetRealmName()].Players[string.sub(text, 1, A - 4)]) then
            local Red, Green, Blue = GearScore_GetQuality(GS_Data[GetRealmName()].Players[string.sub(text, 1, A - 4)]
                                                              .GearScore)
            local PlayerColor = "|cff" .. string.format("%02x%02x%02x", Red * 255, Blue * 255, Green * 255)
            text =
                PlayerColor .. "(" .. GS_Data[GetRealmName()].Players[string.sub(text, 1, A - 4)].GearScore .. ") |r" ..
                    text
        end
...
function GearScore_IsRecordTheSame(Current, Previous)
    if not (Previous.Name) or not (Current.Name) then
        return true;
    end
    if (Previous.GearScore ~= Current.GearScore) then
        return false;
    end
    if (Previous.Date + 10000 <= Current.Date) then
        return false;
    end
    if (Previous.Guild ~= Current.Guild) then
        return false;
    end
    if (Previous.Level ~= Current.Level) then
        return false;
    end
    for i = 1, 18 do
        if (i ~= 4) then
            if (Previous.Equip[i] ~= Current.Equip[i]) then
                return false;
            end
        end
    end
...
function GearScore_SetDetails(tooltip, Name)
    ...
    for i = 1, 18 do
        if not (i == 4) then
            local ItemName, ItemLink, ItemRarity, ItemLevel, ItemMinLevel, ItemType, ItemSubType, ItemStackCount,
                ItemEquipLoc, ItemTexture = GetItemInfo("item:" .. GS_Data[GetRealmName()].Players[Name].Equip[i])
            if (ItemLink) then
...
function GearScore_HookItem(ItemName, ItemLink, Tooltip)
    ...
    local TokenLink, TokenNumber = GearScore_GetItemCode(ItemLink)
    if not (IsEquippableItem(ItemLink)) and not GS_Tokens[TokenNumber] then
        return;
    end
    ...
    if (ItemScore >= 0) then
        if (GS_Settings["Item"] == 1) then
            ...
            if (GS_Settings["ML"] == 1) then
                GearScore_EquipCompare(Tooltip, ItemScore, EquipLoc, ItemLink);
            end
        end
    else
...
function GS_SPAM(Command)
    local tbl = {}
    for v in string.gmatch(Command, "[^ ]+") do
        tinsert(tbl, v);
    end
    ...
    if (strlower(Command) == "party") then
        local GspamDatabase = GearScore_BuildDatabase("Party");
        table.sort(GspamDatabase, function(a, b)
    ...
    if (tbl[1] == "party") or (tbl[1] == "raid") then
        if (tbl[2]) then
            ...
            if (tbl[2] == "Party") then
                local GspamDatabase = GearScore_BuildDatabase(tbl[1]);
                table.sort(GspamDatabase, function(a, b)
                    return a.GearScore > b.GearScore
                end);
                GearScore_SendSpamReport("PARTY", nil, GspamDatabase);
            ...
            if (tbl[2] == "Guild") then
                ...
                GearScore_SendSpamReport("GUILD", nil, GspamDatabase);
            end
            if (tbl[2] == "Officer") then
                local GspamDatabase = GearScore_BuildDatabase(tbl[1]);
                table.sort(GspamDatabase, function(a, b)
                    return a.GearScore > b.GearScore
                end);
                GearScore_SendSpamReport("OFFICER", nil, GspamDatabase);
            end
            if (tbl[2] == "Say") then
                local GspamDatabase = GearScore_BuildDatabase(tbl[1]);
                table.sort(GspamDatabase, function(a, b)
                ...
                GearScore_SendSpamReport("SAY", nil, GspamDatabase);
            end
            if (tbl[2] == "Whisper") then
                local GspamDatabase = GearScore_BuildDatabase(tbl[1]);
                table.sort(GspamDatabase, function(a, b)
                    return a.GearScore > b.GearScore
                end);
                GearScore_SendSpamReport("WHISPER", tbl[3], GspamDatabase);
            end
            if (tbl[2] == "Channel") then
                local GspamDatabase = GearScore_BuildDatabase(tbl[1]);
                table.sort(GspamDatabase, function(a, b)
                    return a.GearScore > b.GearScore
                end);
                GearScore_SendSpamReport("CHANNEL", tbl[3], GspamDatabase);
            end
        end
    end
end
...
function GS_BANSET(Command)
    if not (GS_Settings["BlackList"]) then
        GS_Settings["BlackList"] = {};
    end
    if (GS_Settings["BlackList"][Command]) then
        GS_Settings["BlackList"][Command] = nil;
        print(Command .. " removed from GearScore's Communication Ban list.")
    else
        GS_Settings["BlackList"][Command] = 1;
        print(Command .. " added to GearScore's Communication Ban list.");
    end
end
...
function GS_MANSET(Command)
    ...
    if (strlower(Command) == "describe") then
        GS_Settings["Description"] = GS_Settings["Description"] * -1;
        if (GS_Settings["Description"] == 1) then
            print("Descriptions: On");
        else
            print("Descriptions: Off");
        end
        return;
    end
    ...
    if (strlower(Command) == "communicate") then
        GS_Settings["Communication"] = GS_Settings["Communication"] * -1;
        if (GS_Settings["Communication"] == 1) then
            print("Communication: On");
        else
            print("Communication: Off");
        end
        return;
    end
    ...
    if (strlower(Command) == "date") then
        GS_Settings["Date"] = GS_Settings["Date"] * -1;
        if (GS_Settings["Date"] == 1) then
            print("Date/Time: On");
        else
            print("Date/Time: Off");
    ...
    if (strlower(Command) == "chat") then
        GS_Settings["CHAT"] = GS_Settings["CHAT"] * -1;
        if (GS_Settings["CHAT"] == 1) then
            print("Chat Scores: On");
        else
            print("ChatScores: Off");
        end
        return;
    end
    if (strlower(Command) == "time") then
        GS_Settings["Date"] = GS_Settings["Date"] * -1;
        if (GS_Settings["Date"] == 1) then
            print("Date/Time: On");
        else
    ...
    if (strlower(Command) == "ml") then
        GS_Settings["ML"] = GS_Settings["ML"] * -1;
        if (GS_Settings["ML"] == 1) then
            print("MasterLooting: On");
        else
            print("MasterLooting: Off");
        end
        return;
    end
    if (strlower(Command) == "detail") then
    ...
    if (strlower(Command) == "details") then
        GS_Settings["Detail"] = GS_Settings["Detail"] * -1;
        if (GS_Settings["Detail"] == 1) then
            print("Details: On");
        else
    ...
    if (strlower(Command) == "reset") then
        GS_Settings = GS_DefaultSettings;
        print("All Settings returned to Default");
        return
    end
    if (strlower(Command) == "purge") then
        print("WARNING! This will remove your entire GearScore Database. To continue please type '/gset purge 314159265");
        return;
    end
    if (strlower(Command) == "purge 314159265") then
        GS_Data = nil;
        ReloadUI();
        return;
    end
    ...
    if (strlower(tbl[1]) == "transmit") and (tbl[2]) then
        ...
        if GS_ExchangeDatabase then
            print("You are already transmitting your database!");
            return;
        end
...
function GS_SCANSET(Command)
    if (UnitName("target")) and (Command == "") then
        if not (UnitIsPlayer("target")) then
            GearScore_DisplayUnit(UnitName("player"))
        else
            GearScore_DisplayUnit(UnitName("target"));
        end
    else
        if (Command == "") then
            Command = UnitName("player");
        end
        if (strlen(strupper(string.sub(Command, 1, 1)) .. strlower(string.sub(Command, 2))) ~= strlen(Command)) then
            GearScore_DisplayUnit(Command);
        end
...
function GearScore_GetRaidColor(Raid, Score)
    ...
    if ((Raid - Score) >= 200) then
        return 1, 0, 0;
    end
    if ((Score - Raid) >= 400) then
        return 0, 1, 0;
    end
    if ((Score - Raid) >= 0) and ((Score - Raid) <= 300) then
        return 1, 1, 0;
    end
    if ((Score - Raid) >= 0) and ((Score - Raid) > 300) then
        return (400 - (Score - Raid)) / 200, 1, 0;
    end
    if ((Raid - Score) < 200) then
        return 1, ((Score - (Raid - 200)) / 200), 0;
    end
    return 0, 0, 0
end
...
function GearScore_DisplayUnit(Name, Auto)
    ...
    if Name == UnitName("target") then
        GS_Model:SetUnit("target");
    else
        GS_Model:SetUnit("player");
    end
    GS_Model:Undress()
    ...
    if (GS_Data[GetRealmName()].Players[Name]) then
        for i = 1, 18 do
            if (i ~= 4) then
                ...
                if (ItemTexture) then
                    ...
                    backdrop = {
                        bgFile = GS_TextureFiles[i]
                    };
                    _G["GS_Frame" .. i]:SetBackdrop(backdrop);
                end
            end
        end
        GS_InfoText:SetText(GS_Data[GetRealmName()].Players[Name].Level .. " " ..
                                GS_Races[GS_Data[GetRealmName()].Players[Name].Race] .. " " ..
                                GS_Classes[GS_Data[GetRealmName()].Players[Name].Class])
        GS_InfoText:SetTextColor(GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Name].Class]].Red,
            GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Name].Class]].Green,
            GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Name].Class]].Blue, 1)
        GS_NameText:SetText(GS_Data[GetRealmName()].Players[Name].Name)
        GS_NameText:SetTextColor(GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Name].Class]].Red,
            GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Name].Class]].Green,
            GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Name].Class]].Blue, 1)
        ...
        GS_GuildText:SetTextColor(GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Name].Class]].Red,
            GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Name].Class]].Green,
            GS_ClassInfo[GS_Classes[GS_Data[GetRealmName()].Players[Name].Class]].Blue, 1)
        ...
        local MyDateText = Date .. " days ago";
        if (tonumber(Date) == 0) then
            MyDateText = "Today";
        end
        GS_DateText:SetText("Scanned " .. ColorStringDate .. MyDateText .. "|r  by  " ..
                                GS_Data[GetRealmName()].Players[Name].Scanned)
...
function GearScore_ShowOptions()
    ...
    GS_Displayed = nil
    if (GS_Settings["Restrict"] == 1) then
        GS_None:SetChecked(true);
        GS_Light:SetChecked(false);
        GS_Heavy:SetChecked(false);
    end
    if (GS_Settings["Restrict"] == 2) then
        GS_Light:SetChecked(true);
        GS_None:SetChecked(false);
        GS_Heavy:SetChecked(false);
    end
    if (GS_Settings["Restrict"] == 3) then
        GS_Heavy:SetChecked(true);
        GS_Light:SetChecked(false);
        GS_None:SetChecked(false);
    end
    if (GS_Settings["Player"] == 1) then
        GS_ShowPlayerCheck:SetChecked(true);
    else
        GS_ShowPlayerCheck:SetChecked(false);
    end
    if (GS_Settings["Item"] == 1) then
        GS_ShowItemCheck:SetChecked(true);
    else
        GS_ShowItemCheck:SetChecked(false);
    end
    if (GS_Settings["Detail"] == 1) then
        GS_DetailCheck:SetChecked(true);
    else
        GS_DetailCheck:SetChecked(false);
    end
    if (GS_Settings["Level"] == 1) then
        GS_LevelCheck:SetChecked(true);
    else
        GS_LevelCheck:SetChecked(false);
    end
    if (GS_Settings["AutoPrune"] == 1) then
        GS_PruneCheck:SetChecked(true);
    else
        GS_PruneCheck:SetChecked(false);
    end
    if (GS_Settings["KeepFaction"] == 1) then
        GS_FactionCheck:SetChecked(true);
    else
        GS_FactionCheck:SetChecked(false);
    end
    if (GS_Settings["ML"] == 1) then
        GS_MasterlootCheck:SetChecked(true);
    else
        GS_MasterlootCheck:SetChecked(false);
    end
    if (GS_Settings["CHAT"] == 1) then
        GS_ChatCheck:SetChecked(true);
    else
        GS_ChatCheck:SetChecked(false);
    end
    ...
    for i = 1, 4 do
        _G["GS_SpecFontString" .. i]:Hide();
        _G["GS_SpecScoreCheck" .. i]:Hide();
    end
    for i, v in ipairs(GearScoreClassSpecList[englishClass]) do
        ...
        _G["GS_SpecFontString" .. i]:Show();
        _G["GS_SpecScoreCheck" .. i]:Show()
        if not (GS_Settings["ShowSpecScores"]) then
            GS_Settings["ShowSpecScores"] = {};
        end
        if not (GS_Settings["ShowSpecScores"][GearScoreClassSpecList[englishClass][i]]) then
            GS_Settings["ShowSpecScores"][GearScoreClassSpecList[englishClass][i]] = 1;
        end
        if (GS_Settings["ShowSpecScores"][GearScoreClassSpecList[englishClass][i]] == 1) then
            _G["GS_SpecScoreCheck" .. i]:SetChecked(1);
        else
            _G["GS_SpecScoreCheck" .. i]:SetChecked(0);
        end
    end

    GS_Displayed = 1;
    GS_OptionsFrame:Show();
    GS_GearFrame:Hide();
    GS_ExPFrame:Hide()
end

function GearScore_HideOptions()
    if (GS_ShowItemCheck:GetChecked()) then
        GS_Settings["Item"] = 1;
    else
        GS_Settings["Item"] = -1;
    end
    if (GS_None:GetChecked()) then
        GearScore_SetNone();
    end
    if (GS_Light:GetChecked()) then
        GearScore_SetLight();
    end
    if (GS_Heavy:GetChecked()) then
        GearScore_SetHeavy();
    end
    if (GS_ShowPlayerCheck:GetChecked()) then
        GS_Settings["Player"] = 1;
    else
        GS_Settings["Player"] = -1;
    end
    if (GS_DetailCheck:GetChecked()) then
        GS_Settings["Detail"] = 1;
    else
        GS_Settings["Detail"] = -1;
    end
    if (GS_LevelCheck:GetChecked()) then
        GS_Settings["Level"] = 1;
    else
        GS_Settings["Level"] = -1;
    end
    if (GS_ChatCheck:GetChecked()) then
        GS_Settings["CHAT"] = 1;
    else
        GS_Settings["CHAT"] = -1;
    end
    ...
    if (GS_PruneCheck:GetChecked()) then
        GS_Settings["AutoPrune"] = 1;
    else
        GS_Settings["AutoPrune"] = -1;
    end
    if (GS_FactionCheck:GetChecked()) then
        GS_Settings["KeepFaction"] = 1;
    else
        GS_Settings["KeepFaction"] = -1;
    end
    if (GS_MasterlootCheck:GetChecked()) then
        GS_Settings["ML"] = 1;
    else
        GS_Settings["ML"] = -1;
    end
    ...
    local class, englishClass = UnitClass("player")
    for i, v in ipairs(GearScoreClassSpecList[englishClass]) do
        if (_G["GS_SpecScoreCheck" .. i]:GetChecked()) then
            GS_Settings["ShowSpecScores"][GearScoreClassSpecList[englishClass][i]] = 1;
        else
            GS_Settings["ShowSpecScores"][GearScoreClassSpecList[englishClass][i]] = 0;
        end
    end
...
function GearScore_DisplayDatabase(Group, SortType, Auto, GSX_StartPage)
    ...
    if not (GS_StartPage) or (GS_StartPage < 0) then
        GS_StartPage = 0;
    end
    ...
    local tooltip = LibQTip:Acquire("GearScoreTooltip", 10, "CENTER", "CENTER", "CENTER", "CENTER", "CENTER", "CENTER",
    ...
    tooltip:SetScale(1)
    tooltip:AddLine('#', 'GearScore', 'Name', "iLevel", 'Level', 'Race', 'Class', 'Guild', 'Days Old', 'Sent By');
    tooltip:AddSeparator(1, 1, 1, 1)
    tooltip:SetColumnLayout(10, "CENTER", "CENTER", "LEFT", "CENTER", "CENTER", "LEFT", "LEFT", "CENTER", "CENTER",
        "LEFT")
    ...
    if not (GSX_DataBase) then
        GSX_DataBase = {};
        GSX_DataBase = GearScore_BuildDatabase(Group);
        Auto = 0;
    end

    if not (GS_SortDirection) then
        GS_SortDirection = {};
    end
    if (Auto ~= 1) then
        if (GS_SortDirection[SortType]) then
            GS_SortDirection[SortType] = GS_SortDirection[SortType] * -1;
        else
            GS_SortDirection[SortType] = 1;
        end
        if (SortType == "Name") then
            GS_HighlightedColNum = 3;
            if (GS_SortDirection[SortType] == 1) then
                table.sort(GSX_DataBase, function(a, b)
        ...
        if (SortType == "GearScore") then
            GS_HighlightedColNum = 2;
            if (GS_SortDirection[SortType] == 1) then
                table.sort(GSX_DataBase, function(a, b)
                    return a.GearScore > b.GearScore
                end);
        ...
        if (SortType == "iLevel") then
            GS_HighlightedColNum = 4;
            if (GS_SortDirection[SortType] == 1) then
                table.sort(GSX_DataBase, function(a, b)
        ...
        if (SortType == "Guild") then
            GS_HighlightedColNum = 8;
            if (GS_SortDirection[SortType] == 1) then
                table.sort(GSX_DataBase, function(a, b)
        ...
        if (SortType == "Class") then
            GS_HighlightedColNum = 7;
            if (GS_SortDirection[SortType] == 1) then
                table.sort(GSX_DataBase, function(a, b)
                    return GS_Classes[a.Class] < GS_Classes[b.Class]
                end);
            else
                table.sort(GSX_DataBase, function(a, b)
                    return GS_Classes[a.Class] > GS_Classes[b.Class]
        ...
        if (SortType == "Race") then
            GS_HighlightedColNum = 6;
            if (GS_SortDirection[SortType] == 1) then
                table.sort(GSX_DataBase, function(a, b)
                    return GS_Races[a.Race] < GS_Races[b.Race]
                end);
            else
                table.sort(GSX_DataBase, function(a, b)
                    return GS_Races[a.Race] > GS_Races[b.Race]
        ...
        if (SortType == "Scanned") then
            GS_HighlightedColNum = 10;
            if (GS_SortDirection[SortType] == 1) then
                table.sort(GSX_DataBase, function(a, b)
    ...
    if (GS_StartPage > (#(GSX_DataBase))) then
        GS_StartPage = GS_StartPage - 25;
    end
    ...
    for i, v in pairs(GSX_DataBase) do
        if (i > GS_StartPage) then
            ...
            Recount = Recount + 1
            if (v.Faction == "H") then
                FactionColor = "|cff" .. string.format("%02x%02x%02x", 1 * 255, 0 * 255, 0 * 255);
            else
                FactionColor = "|cff" .. string.format("%02x%02x%02x", 0, 162, 255);
            end
            ColorString1 = "|cff" .. string.format("%02x%02x%02x", GS_ClassInfo[GS_Classes[v.Class]].Red * 255,
                GS_ClassInfo[GS_Classes[v.Class]].Green * 255, GS_ClassInfo[GS_Classes[v.Class]].Blue * 255)
            ...
            tooltip:AddLine(Recount, ColorString2 .. v.GearScore, ColorString1 .. v.Name, v.Average,
                ColorString1 .. v.Level, ColorString1 .. GS_Races[v.Race], ColorString1 .. GS_Classes[v.Class],
                FactionColor .. "<" .. v.Guild .. ">", ColorStringDate .. NowDate, v.Scanned)
            if (i >= (GS_StartPage + 25)) then
                break
            end
            -- else
...
function GearScore_HideDatabase(erase)
    ...
    if (GS_ReportFrame:IsVisible()) then
        keepreport = 1;
    end
    ...
    if (keepreport == 1) then
        GS_ReportFrame:Show();
    end
    if not (erase) then
        GS_DatabaseDisplayed = nil;
        GSX_DataBase = nil;
    end
...
function GearScore_BuildDatabase(Group, Auto)
    ...
    if (Group == "Party") then
        ...
        for i = 1, PartySize do
            if (GS_Data[GetRealmName()].Players[UnitName(GroupType .. i)]) then
                count = count + 1;
                GSL_DataBase[count] = GS_Data[GetRealmName()].Players[UnitName(GroupType .. i)];
            else
                -- GearScore_Request(UnitName(GroupType..i))
            end
        end
        if (GroupType == "party") then
            GSL_DataBase[count + 1] = GS_Data[GetRealmName()].Players[UnitName("player")];
        end
    end
    if (Group == "All") then
        count = 0;
    ...
    if (Group == "Guild") then
        ...
        for i = 1, GetNumGuildMembers(1) do
            if (GS_Data[GetRealmName()].Players[GetGuildRosterInfo(i)]) then
                GSL_DataBase[count] = GS_Data[GetRealmName()].Players[GetGuildRosterInfo(i)];
                count = count + 1;
            end
        end
    end
    if (Group == "Search") then
        count = 0;
        for i, v in pairs(GS_Data[GetRealmName()].Players) do
            local DataString = tostring(v.GearScore .. v.Name .. v.Level .. v.Guild .. GS_Classes[v.Class] ..
                                            GS_Races[v.Race]);
    ...
    if (Group == "Friends") then
        ...
        for i = 1, GetNumFriends(1) do
            if (GS_Data[GetRealmName()].Players[GetFriendInfo(i)]) then
                GSL_DataBase[count] = GS_Data[GetRealmName()].Players[GetFriendInfo(i)];
                count = count + 1;
            end
        end
    end
    ...
    if Group == "All" then
        GSDatabaseInfoString:SetText("Database: " .. count .. " entries. (Approx " ..
                                         floor(0.8372131704586988304093567251462 * count) .. "Kb)");
        GS_Settings["DatabaseSize"] = count;
    else
        if GS_Settings["DatabaseSize"] then
            GSDatabaseInfoString:SetText("Database: " .. GS_Settings["DatabaseSize"] .. " entries. (Approx " ..
                                             floor(0.8372131704586988304093567251462 * GS_Settings["DatabaseSize"]) ..
                                             "Kb)");
        end
    end
    return GSL_DataBase
...
function GearScore_SendReport(Manual, G_Target, G_Who, G_Direction)
    ...
    local Direction = "";
    local Extra = ""
    if (GSXSayCheck:GetChecked()) then
        Target = "SAY";
    end
    if (GSXPartyCheck:GetChecked()) then
        Target = "PARTY";
    end
    if (GSXRaidCheck:GetChecked()) then
        Target = "RAID";
    end
    if (GSXGuildCheck:GetChecked()) then
        Target = "GUILD";
    end
    if (GSXOfficerCheck:GetChecked()) then
        Target = "OFFICER";
    end
    if (GSXWhisperCheck:GetChecked()) then
        Target = "WHISPER";
        Who = GSX_WhisperEditBox:GetText();
    end
    if (GSXWhisperTargetCheck:GetChecked()) then
        Target = "WHISPER";
        Who = UnitName("target");
    end
    if (GSXChannelCheck:GetChecked()) then
        Target = "CHANNEL";
        Who = GSX_ChannelEditBox:GetText();
    end

    if (Target == "") then
        print("Please check the box for where you would like the report to go.");
        return;
    end
    if not (Who) then
        return;
    end
    if (GS_SortDirection[GS_SortedType] == 1) then
        Direction = "Top";
    else
        Direction = "Bottom";
    end
    if (GS_DisplayedGroup == "Search") then
        Extra = "'" .. GS_SearchXBox:GetText() .. "'";
    else
        Extra = GS_DisplayedGroup;
    end
    if (GS_DisplayedGroup == "All") then
        Extra = "Entire Database";
    end
    if (Manual) then
        Target = G_Target;
        Who = G_Who;
...
function GearScore_SendSpamReport(Target, Who, Database)
    SendChatMessage("Top" .. " " .. " GearScore Reports for " .. "group" .. " sorted by " .. "GearScore" .. ".", Target,
        nil, Who);
...
function GearScore_DatabaseOnClick(Event, Cell, Misc, Button)
    ...
    if (Button == "RightButton") and (Cell["_line"] > 2) and (Cell["_column"] == 3) and
        (GSX_DataBase[Cell["_line"] - 2 + GS_StartPage]) then
        ChatFrameEditBox:Show();
        ChatFrameEditBox:Insert("/t " .. GSX_DataBase[Cell["_line"] - 2 + GS_StartPage].Name .. " ");
        return;
    end

    if (Cell["_line"] == 1) and (Cell["_column"] == 2) then
        GearScore_DisplayDatabase(GS_DisplayedGroup, "GearScore", nil, GS_StartPage);
        return;
    end
    if (Cell["_line"] == 1) and (Cell["_column"] == 3) then
        GearScore_DisplayDatabase(GS_DisplayedGroup, "Name", nil, GS_StartPage);
        return;
    end
    if (Cell["_line"] == 1) and (Cell["_column"] == 4) then
        GearScore_DisplayDatabase(GS_DisplayedGroup, "iLevel", nil, GS_StartPage);
        return;
    end
    if (Cell["_line"] == 1) and (Cell["_column"] == 5) then
        GearScore_DisplayDatabase(GS_DisplayedGroup, "Level", nil, GS_StartPage);
        return;
    end
    if (Cell["_line"] == 1) and (Cell["_column"] == 6) then
        GearScore_DisplayDatabase(GS_DisplayedGroup, "Race", nil, GS_StartPage);
        return;
    end
    if (Cell["_line"] == 1) and (Cell["_column"] == 7) then
        GearScore_DisplayDatabase(GS_DisplayedGroup, "Class", nil, GS_StartPage);
        return;
    end
    if (Cell["_line"] == 1) and (Cell["_column"] == 8) then
        GearScore_DisplayDatabase(GS_DisplayedGroup, "Guild", nil, GS_StartPage);
        return;
    end
    if (Cell["_line"] == 1) and (Cell["_column"] == 9) then
        GearScore_DisplayDatabase(GS_DisplayedGroup, "Date", nil, GS_StartPage);
        return;
    end
    if (Cell["_line"] == 1) and (Cell["_column"] == 10) then
        GearScore_DisplayDatabase(GS_DisplayedGroup, "Scanned", nil, GS_StartPage);
        return;
    end
    -- print("pie", Cell["_line"], Cell["_column"]) 
    local LineCount = GS_DatabaseFrame.tooltip:GetLineCount();
    ...
    if (Cell["_line"] > 2) and (Cell["_column"] == 3) and (GSX_DataBase[Cell["_line"] - 2]) and (Cell["_line"] ~= 28) then -- GearScore_Send(GSX_DataBase[Cell["_line"]-2+GS_StartPage].Name, "ALL"); 
        GearScore_DisplayUnit(GSX_DataBase[Cell["_line"] - 2 + GS_StartPage].Name);
        return;
    end
    if (Cell["_line"] > 2) and (Cell["_column"] == 6) and (GSX_DataBase[Cell["_line"] - 2]) then
        GS_SearchXBox:SetText(GS_Races[(GSX_DataBase[Cell["_line"] - 2 + GS_StartPage].Race)]);
        GearScore_HideDatabase();
        GearScore_DisplayDatabase("Search");
        return;
    end
    if (Cell["_line"] > 2) and (Cell["_column"] == 7) and (GSX_DataBase[Cell["_line"] - 2]) then
        GS_SearchXBox:SetText(GS_Classes[(GSX_DataBase[Cell["_line"] - 2 + GS_StartPage].Class)]);
        GearScore_HideDatabase();
        GearScore_DisplayDatabase("Search");
        return;
    end
    if (Cell["_line"] > 2) and (Cell["_column"] == 8) and (GSX_DataBase[Cell["_line"] - 2]) then
        GS_SearchXBox:SetText((GSX_DataBase[Cell["_line"] - 2 + GS_StartPage].Guild));
        GearScore_HideDatabase();
        GearScore_DisplayDatabase("Search");
        return;
    end
...
if not GearScore_TimerFrame then
    GearScore_TimerFrame = CreateFrame("Frame")
end
...
function GearScoreCalculateEXP()
    ...
    if GS_Settings["BackString"] then
        GS_Settings["BackString"] = nil;
    end
    ...
    if not (GS_Data[GetRealmName()]["CurrentPlayer"]) then
        GS_Data[GetRealmName()]["CurrentPlayer"] = {};
    end
...
function GS_DisplayXP(Name)
    ...
    for i, v in ipairs(GS_InstanceOrder) do
        ...
        if (PPValue < 50) then
            red, green, blue = 1, (PPValue / 50), (PPValue / 100);
        end
        if (PPValue >= 50) then
            red, green, blue = 1 - ((PPValue - 50) * 0.02), 1, 0.5 - ((PPValue - 50) * 0.01);
        end
        _G["GS_XpBar" .. barcount]:SetStatusBarColor(red, green, blue)
        _G["GS_XpBar" .. barcount .. "PercentText"]:SetText(PPValue .. "%")
        if not (UnitName("target")) then
            _G["GS_XpBar" .. barcount .. "PercentText"]:SetText("No Target");
        end
